...
export function generateHeader(uid, state, result, winningLang) {
  ...
  if (countsAsWin && (state.totalWins || 0) > 0) {
    ...
    winsText =
      `<text x="${SVG_W - 50}" y="${HDR_TOP + 68}" text-anchor="end" font-size="14" font-weight="800" fill="#4ade80" style="opacity:0;animation:co${uid} .5s ${outDelay}s both">${prevWins}</text>` +
      `<text x="${SVG_W - 50}" y="${HDR_TOP + 68}" text-anchor="end" font-size="14" font-weight="800" fill="#4ade80" style="opacity:1;animation:ci${uid} .5s ${inDelay}s both">${wonTotal}</text>`;
  } else {
    winsText = `<text x="${SVG_W - 50}" y="${HDR_TOP + 68}" text-anchor="end" font-size="14" font-weight="800" fill="#4ade80">${wonTotal}</text>`;
  }
