...
import { FILLERS, NM_FILLERS_EXTRA, CH } from './constants.js';

export function generateCSS(uid, result) {
  const { isWin, nearMissCol } = result;
  let css = '';
  const bln = `bl${uid}`;
  css += isWin
  ...
  for (let c = 0; c < COLS; c++) {
    ...
    if (c === nearMissCol) {
      ...
      css +=
        `@keyframes ${a}{0%{transform:translateY(-${nmScroll}px)}` +
        `70%{transform:translateY(-${Math.round(FILLERS * CH * 0.1)}px)}` +
  ...
  if (nearMissCol >= 0) {
    css +=
      `@keyframes sh${uid}{0%{opacity:0;stroke-width:1}` +
    ...
    css +=
      `@keyframes shp${uid}{0%{opacity:0}10%{opacity:.45}50%{opacity:.55}` +
