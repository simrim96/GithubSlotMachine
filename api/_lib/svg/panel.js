...
export function generateResultPanel(uid, isWin, winningLang, fact, repoMatch, owner, ED, result) {
  ...
  if (isWin && winningLang) {
    ...
    const headLine = result.isJackpot ? `🏆 JACKPOT — ${winningLang.name}!`
                  : result.isBigWin ? `💰 BIG WIN — ${winningLang.name}!`
                  : `🎉 ${winningLang.name} WIN!`;
    const headColor = result.isJackpot ? '#ffd700' : result.isBigWin ? '#ffb84d' : '#4ade80';
    ...
    const msgEn = result.nearMissCol >= 0 ? '😱 So close — try again!' : 'Try again, better luck next time!';
    const msgIt = result.nearMissCol >= 0 ? 'Così vicino, ritenta!' : 'Ritenta, sarai più fortunato!';
