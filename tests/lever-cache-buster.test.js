...
describe('Lever Cache Buster (Issue Fix)', () => {
  ...
  it('api/lever.js deve avere una cache ridotta (non più 3600s)', () => {
    const leverContent = readFileSync('./api/lever.js', 'utf-8');
  ...
  it('La regex della leva deve gestire sia URL senza parametri che con cache-buster esistente', () => {
    ...
    expect(test1.replace(leverRegex, 'api/lever?v=999')).toBe(
      'api/lever?v=999'
    );
    expect(test2.replace(leverRegex, 'api/lever?v=999')).toBe(
      'api/lever?v=999'
    );
    expect(test3.replace(leverRegex, 'api/lever?v=999')).toBe(
      'api/lever?v=999'
    );
  });
